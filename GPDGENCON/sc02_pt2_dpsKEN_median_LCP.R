...
load("/home/fas/caccone/apb56/project/GPDGENCON/DPS/CV/RF_pt1.RData")
...
StraightMedUniq <- foreach(r=1:nrow(begin.table), .combine='rbind', .packages=c('raster', 'gdistance')  ,   .inorder=TRUE   ) %dopar% {
  ...
  data.frame(raster::extract(env, spatial.p, fun=median, na.rm=TRUE))
}

gc() 
#end using objects that will be overwritten

StraightMedUniqDF.train <- as.data.frame(StraightMedUniq)

#bind unique coords to unique lines
StraightMedUniqDF.train <- cbind(unique_coords, StraightMedUniqDF.train)

#use left_join to merge tables by coords in order to get the distance for each pair/line
StraightMedDF.train <- left_join(StraightMedUniqDF.train, Train.table, by = c("long1","lat1","long2","lat2"))

#subset to retain only necessary vars (remove long/lat, var1/var2, etc.) before building models
StraightMedDF.train <- StraightMedDF.train[,c(names(env),"value")]

#remove any NAs for random forest
StraightMedDF.train <- StraightMedDF.train[complete.cases(StraightMedDF.train),]
...
#end using objects that will be overwritten
StraightMedUniqDF.test <- as.data.frame(StraightMedUniq)

#bind unique coords to unique lines
StraightMedUniqDF.test <- cbind(unique_coords, StraightMedUniqDF.test)

#use left_join to merge tables by coords in order to get the distance for each pair/line
StraightMedDF.test <- left_join(StraightMedUniqDF.test, Test.table, by = c("long1","lat1","long2","lat2"))

#subset to retain only necessary vars (remove long/lat, var1/var2, etc.) before building models
StraightMedDF.test<- StraightMedDF.test[,c(names(env),"value")]

#remove any NAs for random forest
StraightMedDF.test <- StraightMedDF.test[complete.cases(StraightMedDF.test),]

set.seed(NULL)

#tune RF
tune_x <- StraightMedDF.train[,names(env)]
tune_y <- StraightMedDF.train[,c("value")]
bestmtry <- tuneRF(tune_x, tune_y, stepFactor=1.5, improve=1e-5, ntree=500)
mtry_opt <- bestmtry[,"mtry"][which.min(bestmtry[,"OOBError"])]

Straight_RF = randomForest(value ~ ., importance=TRUE, mtry = mtry_opt, na.action=na.omit, data=StraightMedDF.train)
...
RMSE2 = sqrt(mean((predict(Straight_RF, StraightMedDF.test) - StraightMedDF.test$value)^2))
MAE = mean(abs(Straight_RF$predicted - StraightMedDF.train$value))
MAE2 =  mean(abs(predict(Straight_RF, StraightMedDF.train) - StraightMedDF.train$value))
MAE3 = mean(abs(predict(Straight_RF, StraightMedDF.test) - StraightMedDF.test$value))
Cor1 = cor(predict(Straight_RF, StraightMedDF.train), StraightMedDF.train$value)
Cor2 = cor(predict(Straight_RF, StraightMedDF.test), StraightMedDF.test$value)
...
for (it in 1:2) {
  ...
  UniqueLcpLoop.test <- foreach(r=1:NumPairs.test, .combine='rbind', .packages=c('raster', 'gdistance')  ,   .inorder=TRUE   ) %dopar% {
    ...
    data.frame(raster::extract(env, Ato, fun=Med, na.rm=TRUE))
  }
...
save.image(paste0("/home/fas/caccone/apb56/project/GPDGENCON/DPS/MEDIAN/LinDPSData_afterLCP_Fold",foldnum,".RData"))
...
write.csv(d, paste0("/home/fas/caccone/apb56/project/GPDGENCON/DPS/MEDIAN/LinDisData_Run", foldnum, "_ValidationTable.csv"), row.names =FALSE)
